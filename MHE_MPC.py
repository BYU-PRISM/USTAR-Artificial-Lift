...
mhe_us = [np.array(mpc.u[i].value) for i in range(npx)] # u relative position storage
mhe_vs = [np.array(mpc.v[i].value) for i in range(npx)]
mhe_fs = [np.array(mpc.f[i].value) for i in range(npx)] # dynamic load storage 
...
#     ax = plt.subplot(414)
#     ax.grid()
#     plt.plot(sim_ts[0:i*npt], mhe_hstor[0:i*npt], label = 'mhe_height')
#     plt.plot(sim_ts[0:i*npt], mpc_hs[0:i*npt], label = 'mpc_height')
#     plt.legend()
#     plt.ylabel('Annular Fluid Height')
#     plt.xlabel('Time (sec)')
#     plt.draw()
...
#%%
np.save('MPC_MHE_Control_Results_20npt_10npx.npy', res) 
#%%
# Load dictionary of results
res = np.load('MPC_MHE_Control_Results_20npt_10npx.npy').item()
