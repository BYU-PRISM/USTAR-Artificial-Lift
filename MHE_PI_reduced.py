...
for i in range(loops):
   ...
   # simulate system for 1 second
    sim.solve() # (remote = False) for local
    if i == 0:
    ...
    #Solve
    t_start = time.time()
    mhe.solve() # (remote = False) for local solve
    
    #Pass values to MPC
    pv[i] = mhe.h_mv.NEWVAL
...
#    if i >= 1:
    ...
    sim.SPM_in.value = np.ones(npt_sim)*op[i]
...
#%%
np.save('PI_MHE_Control_Results_10npt_5npt.npy', res) 
#%%
# Load dictionary of results
res = np.load('PI_MHE_Control_Results_10npt_5npt.npy').item()
...
plt.savefig('PI_MHE_Simulation_Timing.png', dpi = 200, transparent = True)
plt.show()
